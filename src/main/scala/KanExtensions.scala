...
object Lan {
  ...
  def toLan[K[?], G[?], H[?], B](nat: [A] => H[A] => K[G[A]])(lan: Lan[G, H, B])(using Functor[K]): K[B] = lan match {
    case Lan.LeftKan(f, v) => nat(v).map(f)
  }

  // fromLan :: (forall a. Lan g h a -> f a) -> h b -> f (g b)
  def fromLan[K[?], G[?], H[?], B](s: [A] => Lan[G, H, A] => K[A])(hb: H[B]): K[G[B]] = s(glan(hb))

  // glan :: h a -> Lan g h (g a)
  def glan[G[?], H[?]]: [A] => H[A] => Lan[G, H, G[A]] = [A] => (ha: H[A]) => Lan.LeftKan[G, H, G[A], A](identity, ha)
}